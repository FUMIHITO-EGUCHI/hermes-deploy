...
# Bearer required — retrieve once with:
#   docker exec hermes cat /opt/data/.api_server_key
# The key is auto-generated by the compose `command:` wrapper on first
# boot and persists in the named volume. To rotate: delete the file +
...
Point at `http://127.0.0.1:8642/v1`. Bearer required — retrieve with
`docker exec hermes cat /opt/data/.api_server_key`. The key is generated
on first boot, persists in the named volume, and rotates only when you
delete the file + restart the gateway.
